...
**Mechanism.**
- The `users` row carries `session_epoch` alongside `password_hash`. A password change or security reset performs both changes in the same MySQL transaction: `password_hash = new_hash` and `session_epoch = session_epoch + 1`. Password state change and invalidation generation are therefore atomic within one datastore.
